          ...
          <div className={styles.emptyCard} role="status">
            ...
            <p className={styles.emptyDesc}>
              {t('track.signInPrompt') ||
                'Enter your application number to verify your current status, document scrutiny, and processing progress.'}
            </p>

            <form
              onSubmit={(e) => {
                e.preventDefault();
                const cleanNo = quickAppNo.trim();
                if (cleanNo) {
                  router.push(`/login?redirect=${encodeURIComponent(`/track?appNo=${cleanNo}`)}`);
                } else {
                  router.push('/login?redirect=/track');
                }
              }}
              style={{ width: '100%', maxWidth: '440px', margin: '16px auto 24px' }}
            >
              <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                <input
                  type="text"
                  value={quickAppNo}
                  onChange={(e) => setQuickAppNo(e.target.value.toUpperCase())}
                  placeholder={t('track.appNoPlaceholder')}
                  className="form-input"
                  style={{ textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: '600' }}
                  aria-label={t('track.applicationNo') || 'Application Number'}
                />
              ...
              <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', lineHeight: '1.4' }}>
                <IconLock size={13} /> {t('track.privacyNote')}
              </p>
            </form>
          ...
          <div className={styles.controlsRight}>
            ...
            <div className={styles.searchBox}>
              ...
              <input
                ...
                placeholder={t('track.searchPlaceholder') || 'Search my applications by number, service or RTO…'}
                value={searchQuery}
